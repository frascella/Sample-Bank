...
?>
<h3><?php _e('User Status', 'user-locker') ?></h3>
<table class="form-table">
<tr>
	<th scope="row"><?php _e('User account disabled', 'user-locker'); ?></th>
	<td><label for="ul_disabled"><input name="ul_disabled" type="checkbox" id="ul_disabled" value="false" <?php checked(true, get_user_option( 'ul_disabled', $user_id, false )); ?> /> <?php _e('Enable/Disable', 'user-locker'); ?></label></td>
</tr>
<tr>
	...
	function manage_users_columns( $columns ) {
		...
		if ( version_compare( $wp_version, '2.7.999', '>' ) ) {
			if ( get_option( 'userlocker_compact_columns' ) ) {
				$columns['userlocker'] = __('Disabled', 'user-locker');
			} else {
