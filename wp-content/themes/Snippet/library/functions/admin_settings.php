...
<h2><?php echo $themename; ?> Theme Options </h2>
    ...
    <div class="introduction">
        ...
        <a class="switch-link" id="master_switch" href="" title="Show/Hide All Options"><span class="pos"><b>+</b></span><span class="neg"><b>&#8211;</b></span> Show/Hide All Options</a>
...
<?php foreach ($options as $value) { 
	...
	switch ( $value['type'] ) {
		...
		?>
		        <input class="text_input" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" value="<?php if ( get_settings( $value['id'] ) != "") { echo htmlentities( stripslashes( get_settings( $value['id'] ))); } else { echo htmlentities(stripslashes($value['std'])); } ?>" />
		<?php
